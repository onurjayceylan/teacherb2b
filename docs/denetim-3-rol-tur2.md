...
| P1-D | **Wise mutabakatı yapısal olarak hep alarm verecek.** Ledger "beklenen Wise bakiyesi"ni −SUM(wise_clearing) diye türetiyor (canlıda −32.00 USD); Wise hesabının dışarıdan fonlanması modellenmediği için gerçek bakiye girildiğinde fark daima devasa ("UYUŞMAZLIK: 132.00 USD") → günlük alarm yorgunluğu, gerçek sızıntı görünmez olur. | Sahip B5 — canlı yaşandı | "Wise'a transfer ettim" fonlama kaydı (platform_cash→wise_clearing) — fark gerçek anomaliyi ölçsün. |
